...
\begin{align}
&\Sigma_{t=0}^{T+1}{\pmb f}^{(t)T}({\pmb \theta}^{(t)}-{\pmb u}) \nonumber\\
&\le \Sigma_{t=0}^{T+1}{\pmb f}^{(t)}({\pmb \theta}^{(t)}) - \Sigma_{t=0}^{T+1}{\pmb f}^{(t)}({\pmb \theta}^{(T+2)}) \nonumber\\
&= \Sigma_{t=0}^{T}({\pmb f}^{(t)}({\pmb \theta}^{(t)})-{\pmb f}^{(t)}({\pmb \theta}^{(T+2)}))+{\pmb f}^{(T+1)}({\pmb \theta}^{(T+1)})-{\pmb f}^{(T+1)}({\pmb \theta}^{(T+2)}) \nonumber\\
&\le \Sigma_{t=0}^{T}({\pmb f}^{(t)}({\pmb \theta}^{(t)})-{\pmb f}^{(t)}({\pmb \theta}^{(T+1)}))+{\pmb f}^{(T+1)}({\pmb \theta}^{(T+1)})-{\pmb f}^{(T+1)}({\pmb \theta}^{(T+2)}) \nonumber\\
&= \Sigma_{t=0}^{T+1}{\pmb f}^{(t)}({\pmb \theta}^{(t)})-{\pmb f}^{(t)}({\pmb \theta}^{(t+1)}) \nonumber\\
&= \Sigma_{t=0}^{T+1}{\pmb f}^{(t)}({\pmb \theta}^{(t)}-{\pmb \theta}^{(t+1)}) \nonumber
\end{align}
\end{proof}
...
\begin{align}
Regret(A) = \Sigma_{t=1}^{T}{\pmb f}^{(t)T}({\pmb \theta}^{(t)}-{\pmb u}) \le 2\sqrt{2\lambda DT} \nonumber
\end{align}
\end{thm}
...
\begin{align}
\Phi^{(t)}({\pmb \theta})=\eta\Sigma_{i=1}^{t}f^{(i)}({\pmb \theta})+R({\pmb \theta})\nonumber
\end{align}
...
## Regret with strictly convex loss function

- If the loss function is a strictly convex function, even smaller regret can be obtained by gradient descent.

- Gradient descent method updates the parameters based on the following update rule.

\begin{align}
&{\pmb \theta}^{'(t)}={\pmb \theta}^{(t-1)}-\eta_{t-1}\nabla{\pmb f}^{(t-1)}({\pmb theta}^{(t-1)})\nonumber\\
&{\pmb \theta}^{(t)}=\argmin_{{\pmb \theta}\in K}\|{\pmb \theta}-{\pmb \theta}^{'(t)}\|_{2}\nonumber
\end{align}

- This algorithm updates the parameters based on the gradient of the current position and maps it to the nearest point on the feasible area.

## Regret with strictly convex loss function

\begin{thm}[Regret with gradient descent for strictly convex loss functions]
Gradient descent method with a learning rate of $\eta^{(t)}=\frac{1}{\alpha t}$ achieves the following regret for any vector ${\pmb u}\in K$.

\begin{align}
Regret(A)=\Sigma_{t=1}^{T}{\pmb f}^{(t)T}({\pmb \theta}^{(t)}-{\pmb u})\le \frac{G^{2}}{2\alpha}(1+\log T)\nonumber
\end{align}

where, G is an upper bound on the Euclidean norm of the gradient.
\end{thm}
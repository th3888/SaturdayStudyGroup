...
\begin{align}
{\pmb \theta}_{t}=\pi_{K}({\pmb \theta}_{t-1}-\eta\nabla f_{t-1}({\pmb \theta}_{t-1})).\nonumber
\end{align}
...
\begin{thm}
Online Gradient Descent with step sizes $\eta_{t}=\frac{1}{\alpha_{t}}$ achives the following guarantee, for all $T\ge 1$
...
\begin{align}
&\|{\pmb \theta}_{t+1}-{\pmb \theta^{*}}\|^{2}\le \|{\pmb \theta}_{t}-{\pmb \theta^{*}}\|^{2}+\eta^{2}_{t+1}\|\nabla_{t}\|^{2}-2\eta_{t+1}\nabla^{\top}_{t}({\pmb \theta}_{t}-{\pmb \theta^{*}}),\nonumber\\
&2\nabla^{\top}_{t}({\pmb \theta}_{t}-{\pmb \theta^{*}})\le \frac{\|{\pmb \theta}_{t}-{\pmb \theta^{*}}\|^{2}-\|{\pmb \theta}_{t+1}-{\pmb \theta^{*}}\|^{2}}{\eta_{t+1}}+\eta_{t+1}G^{2}
\end{align}
